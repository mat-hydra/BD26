...
ggplot(tre_pop, aes(x = data, y = liczba_uczestnikow)) +
  ...
  scale_y_continuous(breaks = seq(min(tre_pop$liczba_uczestnikow), max(tre_pop$liczba_uczestnikow), by = 1)) +
...
ggplot(fin_atr, aes(x = reorder(sponsor, -suma_wplat), y = suma_wplat)) +
  ...
  labs(x = "Sponsor", y = "Łączny przychód z wpisowego (PLN)",) +
  theme_minimal(base_size = 13) + 
...
scale_fill_manual(values = c("orange", "orange3", "orange4")) +
...
ggplot(dane_kraje, aes(x = kraj, y = liczba_zawodow, fill = kraj)) +
  geom_col(color = "black", size = 0.5, show.legend = FALSE) +
  scale_fill_manual(values = c( "orange2", "orange3", "orange4","chocolate", "saddlebrown")) +
  labs(x = "Kraj", y = "Suma liczby zawodów") + theme_minimal(base_size = 13) 
```
...
ggplot(dane_sponsorzy, aes(x = sponsor, y = liczba_zwyciestw, fill = sponsor)) +
  geom_col(color = "black", size = 0.5, show.legend = FALSE) +
  scale_fill_manual(values = c("darkorange", "orange2", "orange3", "orange4", "chocolate", "saddlebrown")) +
