...
W formule sportów naturalnych mistrzem jest **`r mistrze_wyniki %>% filter(kategoria == "naturalne") %>% slice_max(liczba_wygranych) %>% pull(imie) %>% paste(collapse = ", ")`**, osiągając **`r mistrze_wyniki %>% filter(kategoria == "naturalne") %>% slice_max(liczba_wygranych) %>% pull(liczba_wygranych) %>% head(1)`** zwycięstw.
...
Wyniki wykazały, że **`r sum(dane_doping$liczba_przypadkow[dane_doping$nazwa == "Czysty"])`** zawodników rywalizowało uczciwie.
...
Liderem w organizowaniu zawodów jest **`r dane_kraje %>% slice_max(liczba_zawodow) %>% pull(kraj) %>% paste(collapse = ", ")`**, gdzie zorganizowano **`r dane_kraje %>% slice_max(liczba_zawodow) %>% pull(liczba_zawodow) %>% head(1)`** oficjalnych zawodów.
...
Największą liczbą zwycięstw w historii mogą pochwalić się: **`r dane_sponsorzy %>% filter(liczba_zwyciestw == max(liczba_zwyciestw)) %>% pull(sponsor) %>% paste(collapse = ", ")`**. Sponsorzy ci doprowadzili swoich zawodników do podium **`r max(dane_sponsorzy$liczba_zwyciestw)`** razy.
